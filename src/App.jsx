...
function CategoryCardMedia({ category }) {
  const [imageFailed, setImageFailed] = useState(false);
  const image = typeof category.image === 'string' ? category.image.trim() : '';
  if (!image || imageFailed) return <div className="category-card-placeholder" role="img" aria-label={`${category.name} photo coming soon`}><CategoryIcon name={category.icon} size={34} /><span>Photo coming soon</span></div>;
  return <img src={image} alt={category.imageAlt || category.name} loading="lazy" onError={() => setImageFailed(true)} />;
}

function PromotionImage({ promotion }) {
  const [imageFailed, setImageFailed] = useState(false);
  const image = typeof promotion.image === 'string' ? promotion.image.trim() : '';
  useEffect(() => setImageFailed(false), [image]);
  if (!image || imageFailed) {
    return <div className="promotion-image promotion-image-placeholder" role="img" aria-label={`${promotion.title} promotion photo coming soon`}><TicketPercent size={34} /><span>Offer image coming soon</span></div>;
  }
  return <div className="promotion-image"><img src={image} alt={promotion.imageAlt || `${promotion.title} promotion`} loading="lazy" decoding="async" onError={() => setImageFailed(true)} /></div>;
}
...
function AboutBand() {
  ...
  return <section className="about-band"><div className="container about-band-grid"><div className="about-band-image"><img src={homepageContent.aboutImage || imageUrls.interior} alt={homepageContent.aboutImageAlt || 'Naseeb Chapati restaurant dining atmosphere'} loading="lazy" decoding="async" onError={(event) => { if (event.currentTarget.dataset.fallback !== 'true') { event.currentTarget.dataset.fallback = 'true'; event.currentTarget.src = imageUrls.family; } }} /></div><MotionReveal className="about-band-copy" y={18}><p className="eyebrow">{homepageContent.aboutEyebrow || 'About Naseeb Chapati'}</p><h2>{homepageContent.aboutHeading}</h2><p>{homepageContent.aboutText}</p><div className="value-list"><span><BadgeCheck size={18} />Fresh, made-to-order plates</span><span><BadgeCheck size={18} />Authentic recipes and spices</span><span><BadgeCheck size={18} />A welcoming family dining experience</span></div><Button href={homepageContent.aboutButtonUrl || '/about'} variant="primary" icon={ArrowUpRight}>{homepageContent.aboutButtonLabel || 'Learn more about us'}</Button></MotionReveal></div></section>;
}
...
function BestSellers() {
  ...
  const filters = ['All', ...new Set(menuCategories.map((category) => category.name))];
...
const socialPlatformDetails = {
  facebook: { label: 'Facebook', icon: FacebookBrandIcon, href: 'https://www.facebook.com/', description: 'Follow our page for restaurant news, promotions, and daily updates.', cta: 'Like page' },
  instagram: { label: 'Instagram', icon: InstagramBrandIcon, href: 'https://www.instagram.com/', description: 'Explore fresh dishes, restaurant moments, and food photography.', cta: 'Follow us' },
  tiktok: { label: 'TikTok', icon: TikTokBrandIcon, href: 'https://www.tiktok.com/', description: 'Watch our latest food videos, kitchen moments, and behind-the-scenes stories.', cta: 'Watch now' },
  threads: { label: 'Threads', icon: ThreadsBrandIcon, href: 'https://www.threads.net/', description: 'Join our conversations, quick updates, and fresh stories from the restaurant.', cta: 'Follow us' },
...
function SocialSection() {
  ...
  return <section className="social-section" id="social-media"><div className="container social-inner"><MotionReveal className="social-heading" y={14}><p className="eyebrow">Stay connected</p><h2>Follow Our <span>Social Media</span></h2><p>Stay connected with us for the latest dishes, restaurant news, offers, and guest updates.</p></MotionReveal><MotionGroup className="social-links social-card-grid" amount={.14}>{socialCards.map(({ social, platform, details, href }, index) => { const SocialIcon = details.icon; const useSavedCopy = Boolean(social.platform) && social.platform === platform && Boolean(social.description); return <MotionCard as="a" className={`social-card ${platform}`} key={social.id || `${platform}-${index}`} index={index} href={href} target="_blank" rel="noreferrer" aria-label={`${details.label}: ${useSavedCopy && social.ctaLabel ? social.ctaLabel : details.cta}`}><span className="social-card-icon" aria-hidden="true"><SocialIcon size={34} /></span><h3>{details.label}</h3><strong>{social.username || 'Naseeb Chapati'}</strong><p>{useSavedCopy ? social.description : details.description}</p><span className="social-card-cta">{useSavedCopy && social.ctaLabel ? social.ctaLabel : details.cta}<ArrowUpRight size={15} /></span></MotionCard>; })}</MotionGroup></div></section>;
}
