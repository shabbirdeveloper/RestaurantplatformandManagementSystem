...
import { defaultHomepageStats, getHeroTitleFontFamily, heroSlides as defaultHeroSlides, heroTitleFontOptions, normalizeHeroSlides, normalizeHeroTypography, normalizeHistoryMilestones, normalizeSpecialPlattersSettings } from '../data/content';
...
function WebsiteContentPage({ state, commit, notify }) {
  ...
  const [draft, setDraft] = useState(() => ({
    ...
    ...normalizeSpecialPlattersSettings(homepage),
  }));
  ...
  const saveHomepage = () => {
    ...
    const specialPlattersSettings = normalizeSpecialPlattersSettings({
      ...draft,
    ...
    setDraft((current) => ({
      ...
      historyMilestones,
      ...tickerSettings,
      ...specialPlattersSettings,
    }));
    commit({ homepage: { ...draft, ...typography, ...tickerSettings, ...specialPlattersSettings, stats: normalizeStats(draft.stats), heroSlides: slides, historyMilestones } });
    notify('Homepage content, hero slides, Special Platters, ticker, and history timeline published successfully.');
  };
  ...
  const categoryOptions = (state.categories || [])
    ...
    .filter(Boolean);
  const selectedPlatterCount = (state.menuItems || []).filter((item) => (
    ['Published', 'Active', undefined].includes(item.status)
    && String(item.category || '').trim().toLowerCase() === String(draft.specialPlattersCategory || '').trim().toLowerCase()
  )).length;
  ...
  return <section className="admin-content-page"><div className="admin-editor-grid"><div className="admin-editor-main">
    ...
    <div className="admin-panel admin-form-panel"><div className="admin-panel-heading"><div><span className="admin-panel-kicker">Special Platters</span><h2>Homepage platter carousel</h2><p>Choose the menu category used by the carousel. Published dishes from that category appear automatically.</p></div><span className={`admin-status ${draft.showSpecialPlatters !== false ? 'positive' : ''}`}><i />{draft.showSpecialPlatters !== false ? 'Published' : 'Hidden'}</span></div><div className="admin-form admin-editor-form"><div className="admin-two-col"><label>Section label<input value={draft.specialPlattersEyebrow || ''} onChange={(event) => update('specialPlattersEyebrow', event.target.value)} placeholder="Made to share" /></label><label>Section title<input value={draft.specialPlattersTitle || ''} onChange={(event) => update('specialPlattersTitle', event.target.value)} placeholder="Naseeb Special Platters" /></label></div><label>Section subtitle<textarea rows="3" value={draft.specialPlattersSubtitle || ''} onChange={(event) => update('specialPlattersSubtitle', event.target.value)} /></label><div className="admin-two-col"><label>Menu category<select value={draft.specialPlattersCategory || ''} onChange={(event) => update('specialPlattersCategory', event.target.value)}><option value="">Select category</option>{categoryOptions.map((category) => <option key={category} value={category}>{category}</option>)}</select></label><label>Button label<input value={draft.specialPlattersButtonLabel || ''} onChange={(event) => update('specialPlattersButtonLabel', event.target.value)} placeholder="Explore Platters" /></label></div><div className="admin-two-col"><label>Autoplay speed (ms)<input type="number" min="3000" max="12000" step="100" value={draft.specialPlattersSpeed ?? 5800} onChange={(event) => update('specialPlattersSpeed', event.target.value)} /></label><div className="admin-field-summary"><strong>{selectedPlatterCount} published platter {selectedPlatterCount === 1 ? 'dish' : 'dishes'}</strong><small>{selectedPlatterCount ? 'These dishes will rotate in the homepage carousel.' : 'Until dishes are added, the selected category image is used as the section preview.'}</small></div></div><label className="admin-switch-row"><input type="checkbox" checked={draft.showSpecialPlatters !== false} onChange={(event) => update('showSpecialPlatters', event.target.checked)} /><span className="admin-switch" /><span>Show Special Platters section on homepage</span></label><label className="admin-switch-row"><input type="checkbox" checked={draft.specialPlattersAutoplay !== false} onChange={(event) => update('specialPlattersAutoplay', event.target.checked)} /><span className="admin-switch" /><span>Enable carousel autoplay</span></label><small className="admin-field-help">Add platter dishes from Menu Management and assign them to the selected category. The section updates after publishing homepage content.</small></div></div>
