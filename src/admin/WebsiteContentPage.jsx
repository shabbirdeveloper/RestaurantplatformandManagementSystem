...
import { defaultHomepageStats, getHeroTitleFontFamily, heroSlides as defaultHeroSlides, heroTitleFontOptions, normalizeHeroSlides, normalizeHeroTypography } from '../data/content';
...
function WebsiteContentPage({ state, commit, notify }) {
  ...
  const saveHomepage = () => {
    ...
    setDraft((current) => ({ ...current, ...typography }));
    commit({ homepage: { ...draft, ...typography, stats: normalizeStats(draft.stats), heroSlides: slides } });
  ...
  </div><aside className="admin-editor-side"><div className="admin-panel admin-preview-panel"><div className="admin-panel-heading"><div><span className="admin-panel-kicker">Live preview</span><h2>Homepage hero</h2></div><ExternalLink size={16} /></div><div className="admin-hero-preview"><img src={draft.heroSlides.find((slide) => slide.active !== false && slide.desktopImage)?.desktopImage || draft.desktopImage || '/naseeb-chapati-logo.png'} alt="Homepage hero preview" /><div><span>Homepage</span><h3 style={previewTitleStyle}>{draft.heroHeading}</h3><p>{draft.heroText}</p><button type="button">View Menu <ArrowUpRight size={13} /></button></div></div></div><button className="admin-primary-button admin-save-wide" onClick={saveHomepage}><Save size={16} />Publish homepage content</button></aside></div></section>;
}
