...
const resourceConfig = {
  ...
  gallery: { key: 'gallery', title: 'Gallery Items', description: 'Upload images directly, create gallery collections, and publish them to the public gallery.', add: 'Add Gallery Image', search: 'Search gallery...', empty: 'No gallery items match your filters.' },
...
function ManagementPage({ resource, state, commit, notify }) {
  ...
  const saveRecord = (draft) => {
    commit((current) => {
      if (resource === 'social') {
        const withoutDuplicatePlatform = current.social.filter((row) => ids(row) === ids(draft) || inferSocialPlatform(row) !== draft.platform);
        const social = withoutDuplicatePlatform.some((row) => ids(row) === ids(draft))
          ? withoutDuplicatePlatform.map((row) => ids(row) === ids(draft) ? draft : row)
          : [...withoutDuplicatePlatform, draft];
        return { ...current, social, socialSchemaVersion: SOCIAL_ACCOUNT_SCHEMA_VERSION };
      }
      return { ...current, [config.key]: current[config.key].some((row) => ids(row) === ids(draft)) ? current[config.key].map((row) => ids(row) === ids(draft) ? draft : row) : [draft, ...current[config.key]] };
    });
    setDrawer(null);
    notify(resource === 'menu' && draft.status === 'Published' ? `${draft.name} saved and published on the website.` : resource === 'social' ? `${draft.title} link saved and connected to the public website.` : `${config.singular || config.title.slice(0, -1)} saved successfully.`);
  };
...
function renderCells(resource, row, onToggleFeatured) {
  ...
  if (resource === 'categories') return <><td><div className="admin-table-primary"><span className="admin-category-table-image">{row.image ? <img src={row.image} alt="" onError={(event) => { event.currentTarget.hidden = true; }} /> : null}<CategoryIcon name={row.icon} size={18} /></span><span><strong>{row.name}</strong><small>{row.description}</small></span></div></td><td><span className="admin-order-number">{row.order}</span></td><td>{row.featured ? <StatusBadge status="Featured" /> : '—'}</td><td>{row.branches?.length || 0} branches</td><td><StatusBadge status={row.status} /></td></>;
  ...
  if (resource === 'promotions') return <><td><div className="admin-table-primary"><img src={row.image || '/naseeb-chapati-logo.png'} alt={row.imageAlt || `${row.title} promotion`} onError={(event) => { if (event.currentTarget.dataset.fallback !== 'true') { event.currentTarget.dataset.fallback = 'true'; event.currentTarget.src = '/naseeb-chapati-logo.png'; } }} /><span><strong>{row.title}</strong><small>{row.details}</small></span></div></td><td>{row.type}</td><td>{row.startDate || 'Not scheduled'}{row.endDate ? ` – ${row.endDate}` : ''}</td><td>{row.branches}</td><td><StatusBadge status={row.status} /></td></>;
  if (resource === 'gallery') return <><td><div className="admin-table-primary"><img src={row.image || '/naseeb-chapati-logo.png'} alt={row.alt || row.title} onError={(event) => { if (event.currentTarget.dataset.fallback !== 'true') { event.currentTarget.dataset.fallback = 'true'; event.currentTarget.src = '/naseeb-chapati-logo.png'; } }} /><span><strong>{row.title}</strong><small>{row.alt}</small></span></div></td><td>{row.category}</td><td>{row.branch}</td><td>{row.uploadedAt}</td><td><StatusBadge status={row.status} /></td></>;
  ...
  if (resource === 'social') return <><td><div className="admin-table-primary"><span className={`admin-social-icon ${row.platform || row.className || ''}`} aria-hidden="true">{String(row.title || row.label || 'S').slice(0, 1)}</span><span><strong>{row.title}</strong><small>{row.href}</small></span></div></td><td>{row.username || 'Not set'}</td><td>{row.branches?.join(', ') || 'All branches'}</td><td>{row.displayOrder}</td><td><StatusBadge status={row.status} /></td></>;
...
function makeNewRecord(resource, rows = [], categories = []) {
  ...
  if (resource === 'categories') {
    const nextOrder = rows.reduce((highest, row) => Math.max(highest, Number(row.order) || 0), 0) + 1;
    return { id, slug: id, name: 'New category', description: '', image: '', imageAlt: '', storagePath: '', icon: 'utensils', order: nextOrder, status: 'Draft', featured: false, branches: [] };
  }
  ...
  if (resource === 'promotions') return { id, slug: id, title: 'New promotion', details: '', type: 'Percentage discount', startDate: '', endDate: '', branches: 'Selected branches', terms: '', image: '', imageAlt: '', storagePath: '', status: 'Draft', featured: false };
  if (resource === 'gallery') return { id, title: 'New gallery image', category: '', branch: 'All branches', image: '', alt: '', storagePath: '', status: 'Draft', featured: false, uploadedAt: 'Today' };
  ...
  if (resource === 'social') {
    const platform = socialPlatformOrder.find((candidate) => !rows.some((row) => inferSocialPlatform(row) === candidate)) || 'instagram';
    const nextOrder = rows.reduce((highest, row) => Math.max(highest, Number(row.displayOrder) || 0), 0) + 1;
    return createSocialAccount(platform, { id, href: '', username: '', status: 'Inactive', displayOrder: nextOrder });
  }
  return { page: 'New page', path: '/', title: '', description: '', status: 'Draft' };
}

const recordImageConfig = {
  menu: { altKey: 'imageAlt', folder: 'menu', uploadLabel: 'Change menu image', urlLabel: 'Menu image URL', previewLabel: 'Menu item' },
  categories: { altKey: 'imageAlt', folder: 'categories', uploadLabel: 'Change category photo', urlLabel: 'Category image URL', previewLabel: 'Category' },
  branches: { altKey: 'imageAlt', folder: 'branches/profiles', uploadLabel: 'Change branch photo', urlLabel: 'Branch profile image URL', previewLabel: 'Branch' },
  promotions: { altKey: 'imageAlt', folder: 'promotions', uploadLabel: 'Change promotion image', urlLabel: 'Promotion image URL', previewLabel: 'Promotion' },
  gallery: { altKey: 'alt', folder: 'gallery', uploadLabel: 'Change gallery image', urlLabel: 'Gallery image URL', previewLabel: 'Gallery image' },
};
...
function RecordDrawer({ resource, record, onSave, onClose }) {
  ...
  const imageConfig = recordImageConfig[resource];
  ...
  const fields = resource === 'menu' ? [['name', 'Item name'], ['category', 'Category'], ['order', 'Display order'], ['price', 'Base price'], ['status', 'Status'], ['stock', 'Stock status'], ['description', 'Short description'], ['ingredients', 'Ingredients']] : resource === 'categories' ? [['name', 'Category name'], ['imageAlt', 'Category image alt text'], ['icon', 'Category icon'], ['order', 'Display order'], ['status', 'Status'], ['description', 'Description']] : resource === 'branches' ? [['name', 'Branch name'], ['code', 'Branch code'], ['address', 'Full address'], ['imageAlt', 'Profile image alt text'], ['phone', 'Phone'], ['whatsapp', 'WhatsApp'], ['manager', 'Branch manager'], ['status', 'Status']] : resource === 'promotions' ? [['title', 'Promotion title'], ['imageAlt', 'Promotion image alt text'], ['type', 'Promotion type'], ['startDate', 'Start date'], ['endDate', 'End date'], ['branches', 'Applicable branches'], ['status', 'Status'], ['details', 'Short description'], ['terms', 'Terms and conditions']] : resource === 'gallery' ? [['title', 'Title'], ['category', 'Gallery collection'], ['branch', 'Branch'], ['alt', 'Image alt text'], ['status', 'Status']] : resource === 'reviews' ? [['name', 'Customer name'], ['rating', 'Rating'], ['branch', 'Branch'], ['status', 'Status'], ['text', 'Review text']] : resource === 'reservations' ? [['name', 'Customer name'], ['phone', 'Phone'], ['email', 'Email'], ['branch', 'Branch'], ['date', 'Date'], ['time', 'Time'], ['guests', 'Guests'], ['status', 'Status'], ['notes', 'Internal notes']] : resource === 'enquiries' ? [['name', 'Name'], ['email', 'Email'], ['phone', 'Phone'], ['branch', 'Branch'], ['subject', 'Subject'], ['status', 'Status'], ['message', 'Message'], ['notes', 'Internal notes']] : resource === 'social' ? [['platform', 'Platform'], ['href', 'Profile URL'], ['username', 'Username'], ['description', 'Card description'], ['ctaLabel', 'Button label'], ['status', 'Status'], ['displayOrder', 'Display order']] : resource === 'seo' ? [['page', 'Page'], ['path', 'Path'], ['title', 'Meta title'], ['description', 'Meta description'], ['status', 'Status']] : [['name', 'Name'], ['description', 'Description'], ['status', 'Status']];
  ...
  const uploadRecordImage = async (event) => {
    ...
    try {
      if (isSupabaseConfigured) {
        const uploaded = await uploadMediaFile(file, imageConfig?.folder || 'gallery');
  ...
  const submitRecord = (event) => {
    ...
    if (resource === 'social') {
      const platform = draft.platform || draft.className || 'instagram';
      const preset = socialPlatformPresets[platform] || socialPlatformPresets.instagram;
      normalizedDraft.platform = platform;
      normalizedDraft.className = platform;
      normalizedDraft.label = preset.label;
      normalizedDraft.title = preset.label;
      normalizedDraft.href = String(draft.href || '').trim();
      normalizedDraft.displayOrder = draft.displayOrder === '' ? 1 : Math.max(1, Number(draft.displayOrder) || 1);
    }
    ...
    <aside className="admin-drawer">
      ...
      {imageConfig && <>
        <div className={`admin-drawer-image ${uploadingImage ? 'is-uploading' : ''}`}>
          <img src={draft.image || '/naseeb-chapati-logo.png'} alt={draft[imageConfig.altKey] || `${draft.name || draft.title || imageConfig.previewLabel} preview`} onError={(event) => { if (event.currentTarget.dataset.fallback !== 'true') { event.currentTarget.dataset.fallback = 'true'; event.currentTarget.src = '/naseeb-chapati-logo.png'; } }} />
          <label className="admin-image-edit"><Edit3 size={14} />{uploadingImage ? 'Uploading…' : imageConfig.uploadLabel}<input type="file" accept="image/png,image/jpeg,image/webp,image/avif" onChange={uploadRecordImage} disabled={uploadingImage} /></label>
        </div>
        <div className="admin-record-image-controls">
          <label>{imageConfig.urlLabel}<input type="url" value={draft.image || ''} onChange={(event) => { update('image', event.target.value); setImageError(''); }} placeholder="Upload an image or paste a public URL" disabled={uploadingImage} /></label>
      ...
      <form className="admin-form admin-drawer-form" onSubmit={submitRecord}>
        <div className="admin-drawer-fields">{fields.map(([key, label]) => {
          ...
          if (key === 'platform') return <label key={key}>{label}<select value={draft.platform || draft.className || 'instagram'} onChange={(event) => { const platform = event.target.value; const preset = socialPlatformPresets[platform]; setDraft((current) => ({ ...current, platform, className: platform, label: preset.label, title: preset.label, description: current.description || preset.description, ctaLabel: current.ctaLabel || preset.ctaLabel })); }} required>{socialPlatformOrder.map((value) => <option key={value} value={value}>{socialPlatformPresets[value].label}</option>)}</select><small>Controls the official logo, card colour, and platform name on the public website.</small></label>;
          if (key === 'category' && resource === 'gallery') return <label key={key}>{label}<input list="gallery-collection-suggestions" value={draft.category || ''} onChange={(event) => update('category', event.target.value)} placeholder="For example: Food, Interior, Events" required /><datalist id="gallery-collection-suggestions">{['Food', 'Interior', 'Events', 'Customers', 'Behind the Scenes', 'Staff', 'Kitchen'].map((option) => <option key={option} value={option} />)}</datalist><small>Enter any collection name. The public gallery filter is created automatically from published gallery records.</small></label>;
          ...
          return <label key={key}>{label}{['status', 'category', 'type', 'stock', 'branch'].includes(key) ? <select value={draft[key] ?? ''} onChange={(event) => update(key, event.target.value)} required={resource === 'social' && key === 'status'}><option value="">Select {label.toLowerCase()}</option>{(key === 'status' ? resource === 'social' ? ['Active', 'Inactive'] : ['Draft', 'Published', 'Active', 'Inactive', 'Archived', 'Pending', 'Approved', 'Confirmed', 'Resolved'] : key === 'category' ? ['Chapati and Bread', 'Breakfast', 'Chicken Dishes', 'Mutton Dishes', 'Biryani and Rice', 'Curry', 'Tandoori and Grill', 'Family Platters', 'Fast Food', 'Hot Drinks', 'Cold Drinks', 'Desserts'] : key === 'branch' ? ['All branches', 'Pasir Gudang', 'Ayer Hitam', 'Angsana JB Mall'] : key === 'stock' ? ['Available', 'Sold out', 'Scheduled'] : ['Combo meal', 'Percentage discount', 'Fixed discount', 'Weekend promotion']).map((option) => <option key={option}>{option}</option>)}</select> : key === 'description' || key === 'ingredients' || key === 'terms' || key === 'text' || key === 'message' || key === 'notes' ? <textarea value={draft[key] ?? ''} onChange={(event) => update(key, event.target.value)} rows="3" /> : <input type={['price', 'rating', 'guests', 'displayOrder', 'order'].includes(key) ? 'number' : ['startDate', 'endDate', 'date'].includes(key) ? 'date' : ['time'].includes(key) ? 'time' : key === 'email' ? 'email' : key === 'href' ? 'url' : 'text'} required={resource === 'social' && key === 'href'} min={key === 'order' || key === 'displayOrder' ? 1 : undefined} step={key === 'order' || key === 'displayOrder' ? 1 : undefined} value={draft[key] ?? ''} onChange={(event) => update(key, event.target.value)} />}</label>;
        })}</div>
