...
function makeNewRecord(resource, rows = [], categories = []) {
  ...
  if (resource === 'social') return { id, label: 'Instagram', title: 'Instagram', platform: 'instagram', href: '', username: '', description: '', ctaLabel: 'Follow us', className: 'instagram', status: 'Inactive', displayOrder: 1, branches: ['All branches'] };
...
function RecordDrawer({ resource, record, onSave, onClose }) {
  ...
  const fields = resource === 'menu' ? [['name', 'Item name'], ['category', 'Category'], ['order', 'Display order'], ['price', 'Base price'], ['status', 'Status'], ['stock', 'Stock status'], ['description', 'Short description'], ['ingredients', 'Ingredients']] : resource === 'categories' ? [['name', 'Category name'], ['imageAlt', 'Category image alt text'], ['icon', 'Category icon'], ['order', 'Display order'], ['status', 'Status'], ['description', 'Description']] : resource === 'branches' ? [['name', 'Branch name'], ['code', 'Branch code'], ['address', 'Full address'], ['imageAlt', 'Profile image alt text'], ['phone', 'Phone'], ['whatsapp', 'WhatsApp'], ['manager', 'Branch manager'], ['status', 'Status']] : resource === 'promotions' ? [['title', 'Promotion title'], ['imageAlt', 'Promotion image alt text'], ['type', 'Promotion type'], ['startDate', 'Start date'], ['endDate', 'End date'], ['branches', 'Applicable branches'], ['status', 'Status'], ['details', 'Short description'], ['terms', 'Terms and conditions']] : resource === 'gallery' ? [['title', 'Title'], ['category', 'Gallery collection'], ['branch', 'Branch'], ['alt', 'Image alt text'], ['status', 'Status']] : resource === 'reviews' ? [['name', 'Customer name'], ['rating', 'Rating'], ['branch', 'Branch'], ['status', 'Status'], ['text', 'Review text']] : resource === 'reservations' ? [['name', 'Customer name'], ['phone', 'Phone'], ['email', 'Email'], ['branch', 'Branch'], ['date', 'Date'], ['time', 'Time'], ['guests', 'Guests'], ['status', 'Status'], ['notes', 'Internal notes']] : resource === 'enquiries' ? [['name', 'Name'], ['email', 'Email'], ['phone', 'Phone'], ['branch', 'Branch'], ['subject', 'Subject'], ['status', 'Status'], ['message', 'Message'], ['notes', 'Internal notes']] : resource === 'social' ? [['platform', 'Platform'], ['title', 'Account title'], ['href', 'Profile URL'], ['username', 'Username'], ['description', 'Card description'], ['ctaLabel', 'Button label'], ['status', 'Status'], ['displayOrder', 'Display order']] : resource === 'seo' ? [['page', 'Page'], ['path', 'Path'], ['title', 'Meta title'], ['description', 'Meta description'], ['status', 'Status']] : [['name', 'Name'], ['description', 'Description'], ['status', 'Status']];
  ...
  const submitRecord = (event) => {
    ...
    if (resource === 'social') {
      const platformLabels = { facebook: 'Facebook', instagram: 'Instagram', tiktok: 'TikTok', threads: 'Threads', google: 'Google Reviews' };
      ...
      normalizedDraft.label = platformLabels[platform] || draft.title || 'Social account';
      normalizedDraft.title = draft.title || normalizedDraft.label;
    ...
    <aside className="admin-drawer">
      ...
      <form className="admin-form admin-drawer-form" onSubmit={submitRecord}>
        <div className="admin-drawer-fields">{fields.map(([key, label]) => {
          ...
          if (key === 'platform') return <label key={key}>{label}<select value={draft.platform || draft.className || 'instagram'} onChange={(event) => { const platform = event.target.value; const titles = { facebook: 'Facebook', instagram: 'Instagram', tiktok: 'TikTok', threads: 'Threads', google: 'Google Reviews' }; setDraft((current) => ({ ...current, platform, className: platform, label: titles[platform], title: titles[platform] })); }} required>{[['instagram', 'Instagram'], ['tiktok', 'TikTok'], ['threads', 'Threads'], ['google', 'Google Reviews'], ['facebook', 'Facebook']].map(([value, optionLabel]) => <option key={value} value={value}>{optionLabel}</option>)}</select><small>Controls the official logo, card colour, and platform name on the public website.</small></label>;
