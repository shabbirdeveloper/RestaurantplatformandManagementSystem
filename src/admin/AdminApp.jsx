...
function RecordDrawer({ resource, record, onSave, onClose }) {
  ...
  const uploadRecordImage = async (event) => {
    ...
    try {
      if (isSupabaseConfigured) {
        const folder = resource === 'branches' ? 'branches/profiles' : resource === 'categories' ? 'categories' : 'menu';
  ...
  return <div className="admin-drawer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && !uploadingImage && onClose()}>
    <aside className="admin-drawer">
      ...
      {['menu', 'categories', 'branches'].includes(resource) && <>
        <div className={`admin-drawer-image ${uploadingImage ? 'is-uploading' : ''}`}>
          <img src={draft.image || '/naseeb-chapati-logo.png'} alt={draft.imageAlt || `${draft.name || (resource === 'branches' ? 'Branch' : resource === 'categories' ? 'Category' : 'Menu item')} preview`} />
          <label className="admin-image-edit"><Edit3 size={14} />{uploadingImage ? 'Uploading…' : resource === 'branches' ? 'Change branch photo' : resource === 'categories' ? 'Change category photo' : 'Change image'}<input type="file" accept="image/png,image/jpeg,image/webp,image/avif" onChange={uploadRecordImage} disabled={uploadingImage} /></label>
        </div>
        <div className="admin-record-image-controls">
          <label>{resource === 'branches' ? 'Branch profile image URL' : resource === 'categories' ? 'Category image URL' : 'Image URL'}<input type="url" value={draft.image || ''} onChange={(event) => { update('image', event.target.value); setImageError(''); }} placeholder="Upload an image or paste a public URL" disabled={uploadingImage} /></label>
